...
def picks(ttrfile, wavepath, refpath):
    ...
    for line in lines[8:]:
        ...
        tr.stats.sac.user2 = uncertainty
        tr.write(filename=fn, format="SAC")

def picks_from_sac(sacfiles, pickpath, unit_evdp="km"):

    try:
        os.makedirs(pickpath)
    except:
        pass
    ...
    tr = read(sacfiles[0])[0]
    phase = tr.stats.sac.kt1
    ...
    for f in sacfiles:
        ...
        try:
            unce = tr.stats.sac.user2
        except:
            unce = 0.1

        try:
            # reftime = reftime_from_trace(tr)
            arrival = tr.stats.sac.t2 - tr.stats.sac.o - tr.stats.sac.t1
        except:
            continue

        nsta += 1

        # lst.append([stla, stlo, stel, arrival, unce])
        lst.append([stla, stlo, 0.0, arrival, unce])
        arr.append(arrival)

    mean = sum(arr)/len(arr)

    fn = pickpath + "/asf" + evtid + ".%s.ttr" % phase
    fp = open(fn, "w")
    fp.write("%d\n" % nsta)
    fp.write("%.4f\t%.4f\t%.4f\n" % (evla, evlo, evdp))
    fp.write("%s\n" % phase)
    for a in lst:
        fp.write("%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n" % (a[0], a[1], a[2], a[3]-mean, a[4]))
    fp.close()
