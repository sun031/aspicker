...
c
      ...
      real ttto,den,err(MAXST),wsp(MAXD),emax,emin
      real evlat,evlon,evdep,sec,scu,pstakn
      character*30 efile
      character aqini*30,aqfini*30,akph*8,ttres*30
      character*8 csta(MAXST)
      real vu(MAXD),erl,errl,errr,zv(MAXST,MAXD),tshift(MAXST)
      real dtcmin,dtcmax,pjgl,dtmx
      integer wsl
      real wm,ws,tshify
c
c
...
c     filenames.
c
      aqini="asi"//efile(4:17)//".aq"
      aqfini="asf"//efile(4:17)//".aq"
c
c     Construct file name for traveltime
c     residual output.
c
      ttres="rts"//efile(4:17)//".ttr"
c
c----------------------------------------------------------------------
c     OBTAIN EVENT  DATA
c----------------------------------------------------------------------
...
c
      ...
      ENDDO
      loff = abs(dtcmin)
      if(abs(dtcmax).gt.abs(dtcmin))loff=abs(dtcmax)
      loff = nint((dtmx+loff)/sampin)+1
      nstk=2*loff+npmx
      lim1 = loff+nstkwb
...
c
      ...
      write(INI,*) ihr,imin,sec, ttto
      write(INI,*) sampin, akph
      DO i=1,nsta
...
c
      ...
      write(FIN,*) ihr,imin,sec, ttto
      write(FIN,*) sampin, akph
      DO i=1,nsta
